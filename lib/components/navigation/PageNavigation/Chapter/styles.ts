...
const getStyles = (theme: Theme): Styles<ChapterClassNames> => {
  const styles: Styles<ChapterClassNames> = {
    Chapter: {
      position: 'relative'
    },
    ...
    Title: {
      display: 'flex',
      padding: `${theme.space(1)}px ${theme.space(2)}px ${theme.space(1)}px ${theme.space(4)}px`,
      margin: 0,
      lineHeight: 1.1,
      cursor: 'pointer',
      textDecoration: 'none',
